...
class SOFollower(Robot):
    ...
    def calibrate(self) -> None:
        ...
        with open('/home/csh/Desktop/calibration_data.json', 'w') as f:
            json.dump(calib_dict, f, indent=2)
            print("Calibration saved to calibration_data.json")
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        """Send action to motors (assumes values are in correct normalization mode)"""
        if not self.is_connected:
            raise DeviceNotConnectedError(f"{self} is not connected.")
        
        goal_pos = {key.removesuffix(".pos"): val for key, val in action.items() if key.endswith(".pos")}
        
        # 직접 전송 (FeetechMotorsBus가 정규화 처리)
        self.bus.sync_write("Goal_Position", goal_pos)
