...
def play_trajectory(
    ...
    print("[PLAY] ✓ 완료")
class SO101FollowJTAcServer(Node):
    def __init__(self, robot, joint_signs, joint_offsets, freq=50, kp=0.3, keep_gripper=0.0, dry_run=False, action_name="/joint_trajectory_controller/follow_joint_trajectory"):
        super().__init__("so101_follow_jt_server")
        self.robot = robot
        self.joint_signs = joint_signs
        self.joint_offsets = joint_offsets
        self.freq = freq
        self.kp = kp
        self.keep_gripper = keep_gripper
    ...
    def goal_cb(self, goal_request):
        ...
        if not goal_request.trajectory.joint_names:
            self.get_logger().error("Goal rejected: empty joint_names")
            return GoalResponse.REJECT

        if not goal_request.trajectory.points:
            self.get_logger().error("Goal rejected: empty points")
            return GoalResponse.REJECT
    ...
    def execute_cb(self, goal_handle):
        ...
        # JointTrajectoryPoint.time_from_start -> seconds array
        times = []
        positions = []
        for p in traj.points:
            t = float(p.time_from_start.sec) + float(p.time_from_start.nanosec) * 1e-9
            times.append(t)
            positions.append(list(p.positions))
        ...
        if times.ndim != 1 or positions.ndim != 2:
            ...
            res.error_code = -1
            res.error_string = "Invalid trajectory format (times/positions dimension)"
            return res
        ...
        if positions.shape[1] != len(joint_names):
            ...
            res = FollowJointTrajectory.Result()
            res.error_code = -1
            res.error_string = "positions dimension != len(joint_names)"
            return res

        # time_from_start가 정렬 안 되어 있을 수 있어 정렬
        order = np.argsort(times)
        times = times[order]
        positions = positions[order]

        # === 여기서 기존 play_trajectory() 재사용 ===
        # play_trajectory는 positions_rad(라디안)을 받아 내부에서 deg로 변환/보정 후 lerobot send_action 함.
        # (FollowJointTrajectory도 기본적으로 rad 단위가 일반적)
        try:
            play_trajectory(
                robot=self.robot,
                ...
                kp=self.kp,
                keep_gripper_deg=self.keep_gripper,
            ...
            res = FollowJointTrajectory.Result()
            res.error_code = -1
            res.error_string = f"Execution exception: {e}"
            return res
...
예시:
  ...
  python %(prog)s --yaml trajectory.yaml --max-vel 20 --kp 0.2
    ...
    parser.add_argument("--as-server", action="store_true", help="FollowJointTrajectory Action Server로 실행")
    parser.add_argument("--action-name", default="/joint_trajectory_controller/follow_joint_trajectory", help="Action name")
    ...
    if not args.as_server and not args.yaml:
        parser.error("--yaml is required unless --as-server is used")
    ...
    if args.dry_run:
        ...
        robot.connect()
        if args.as_server:
            # 서버 모드: yaml 로드/재생 대신 action server spin
            rclpy.init()

            try:
                node = SO101FollowJTAcServer(
                    robot=robot,
                ...
                print("[ROS] Action server spinning...")
                rclpy.spin(node)
            finally:
                rclpy.shutdown()
            return
